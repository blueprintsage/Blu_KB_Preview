...
| PASS-HARDEN-FIGURE-STAGE-BOUNDARIES | Enforce drawing-stage information ceilings and stop Stage 2 at structural lock | GPT | review | Base: corrected Chapter 4 full repo. Documentation/corpus-consumption update only: Stage 2 is now a plain articulated maquette, Stage 3 describes specific form, and Stage 4 presents it. Visual Stage 2 precedents are deferred until the user supplies stronger examples. Schema and tooling gates pass; the archive omits the original first-party source payload/images, so those source/reference gates require a canonical checkout. |
